var Report =
{
    ...
    {
        var tab_name = $( "#accordion-panel" ).children( ".ui-state-active" ).text();
        tab_name += " ";
        tab_name += $( "#hierarchy-panel" ).find( ".ui-state-active" ).text();
        tab_name += " - Cumulative Summary";

        var tab_data_id = Report._makeTabName( 'summary', key );
    ...
    _addInfoTab: function()
    {
        ...
        TableDecorator.loadView( tab_data_id, $( "#" + tabId ).children( "table" ).slice( 1 ) );
    },
    ...
    _loadCodeCoverage: function( inst_id, item_id, withSelect )
    {
        ...
        var tab_data_id = Report._makeTabName( type, inst_id, targetPath );

        if ( tab_data_id in Report.idByName )
        {
            if ( withSelect )
                Report._scrollToLine( Report.idByName[ tab_data_id ], targetLine );
        }
        else
        {
    ...
    _calculateMargins: function()
    {
        ...
        var rightMargin = 34,
        leftMargin = sidebar.length ? 129 : 0,
        windowInnerWidth = window.innerWidth,
        sidebarMaxWidth = windowInnerWidth - rightMargin,
        content = $( '#content' ),
        contentLeftMargin = parseInt( content.css( 'left' ) ),
        contentRightMargin = windowInnerWidth - contentLeftMargin;
